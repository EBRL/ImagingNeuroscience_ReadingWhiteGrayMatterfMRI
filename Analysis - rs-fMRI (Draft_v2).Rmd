...
```{r -- loading atlas labels & behavioral data }
...
```{r -- loading participant WM-GM connectivity matrices }
...
```{r -- visualizing group-averaged white- and gray-matter fuctional connectivity [Supplemental Figure 1]}
...
```{r -- connectivity values filtered by WM tracts }
...
```{r -- transforming & collapsing data across individual matrices }
...
```{r -- visualizing distributions of reading scores & correlation [Figure 3a+b] }
...
```{r -- residualizing WM-GM functional connectivity values }
...
```{r -- filtering BA GM ROIs based on structural circuitry }
...
```{r -- regression - RC = WR x WM-GM ROIs [Table 2] }
...
```{r - visualizing ROI-based connectivity matrix [Figure 3a] }
  data_dir = "~/[INSERT PATH HERE]/Manuscript/DATA/"
...
```{r -- visualizing interaction plots [Figure 3b] }
